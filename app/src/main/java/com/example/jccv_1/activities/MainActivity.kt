        ...
        GlobalScope.launch {
            ...
            if (fechaini != "día/mes/año" && fechafin != "día/mes/año") {
                ...
                if ( inicial.isNotEmpty())
                {
                    ...
                    lista = lista.filter { factura: Facturas -> sdf.parse(factura.fecha) <= sdf.parse(fechafin) && sdf.parse(factura.fecha) >= sdf.parse(fechaini) }
                }
            ...
            if (pagadass == "Y" || pendientess == "Y" || anuladass == "Y" || cfijass == "Y" || plans == "Y") {
                ...
                if (cfijass == "Y") {
                    listaf = (listaf + filtroCfija).sortedByDescending { facturas: Facturas -> sdf.parse(facturas.fecha)  }
                }
                if (plans == "Y") {
